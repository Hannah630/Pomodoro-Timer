...
淺色主題整個是 `base.css` 裡的一個規則，覆寫 **5 個 token**：`--ink`、
...
原本這是 `@media (prefers-color-scheme: light)`，也就是系統說了算。問題是同一份
程式碼在淺色的桌機與深色的手機上長得不一樣，而**「我想要一直是淺色」在那個模型
裡是說不出口的**。

所以主題有三個值：`system` / `light` / `dark`。`system` 不是「沒有選」——它是一個
選擇，而且是預設的那個；分不清這兩者，就沒辦法區分「跟著系統」與「剛好選了跟系統
一樣的顏色」。
...
| 層 | 負責 |
| --- | --- |
| `models/theme.model.ts` | 三個值、預設值。純資料 |
| `services/theme.service.ts` | **把 `system` 解析成 light 或 dark**，記住選擇，變動時通知 |
| `ui/theme-view.ts` | 把解析結果寫成根元素的 `data-theme`，並標出哪顆按鈕被按著 |
| `storage.service.ts` | 驗證讀回來的值 |

解析之所以是**服務**而不是 view 的一個欄位：三個值裡只有兩個是事實，第三個是
要去問機器的問題，而一個頁面在三個地方各問一次 `matchMedia`，就是三個互相走鐘的
機會。系統偏好用 `SchemeQuery` 注入，所以「入夜時手機自己翻成深色」這件事可以在
node 裡測。

**CSS 只有兩個區塊，不是四個。** theme-view 寫進去的是**解析後**的值，永遠不會是
`system`，所以樣式表不需要為「淺色 × 跟隨系統」和「淺色 × 明確指定」各寫一次。
深色是基底，`:root[data-theme='light']` 是唯一的覆寫——跟原本的 media query 版本
一樣多。

代價說清楚：屬性是 JS 寫的，所以在 JS 跑之前那一瞬間，頁面是深色的。因此
`main.ts` 把主題放在**最前面**，`storage.load()` 之後就立刻做，其他東西都排在
它後面。真的看得見閃爍的話，解法是在 `<head>` 放一小段 inline script，但那要把
storage key 複製一份到 HTML 裡，在還沒確認問題存在之前不划算。

另外 `index.html` 的 `theme-color` 與 manifest 的 `theme_color` 仍然跟著系統走
——它們控制的是瀏覽器外框而不是頁面，而且在 iOS standalone 加上
`black-translucent` 之下幾乎沒有作用。列為已知落差。

#### 為什麼沒有 bump 版本

`theme` 是加上去的欄位，有預設值，舊存檔缺它不代表讀不懂。版本號的意思是「這個
build 讀不了那份存檔」，而這不是那種情況——`title` 當初也是這樣加的。

而且 bump 在這裡是**負面**的：認不得的版本一律整份丟掉，所以一個還開著的舊分頁
會為了一個它根本不需要知道的配色設定，把時長與累計次數一起清掉。
...
加了 manifest 與 iOS 的幾個 meta，讓它可以被加到主畫面、以 standalone 開啟。

動機不是「做成 PWA」這個目標本身，而是**上一節那五條在 Safari 分頁裡根本測不
到**：分頁裡瀏覽器自己的上下列蓋住瀏海與 home indicator，四個 inset 全是 0。
從主畫面圖示打開，頁面才拿得到整個螢幕——而那正是原生外殼會給的東西，所以這是
目前唯一能誠實驗證那段 CSS 的方法。開發機是 Windows、手機是 iPhone、手上沒有
Mac，這條路不需要其中任何一樣。

幾個決定：

- **manifest 裡的路徑全用相對值**（`start_url: "."`、圖示不帶前導斜線）。同一
  份檔案要在 Pages 的 `/Pomodoro-Timer/` 與外殼的根目錄下都成立，寫死任何一邊
  都會壞掉。`index.html` 裡那兩個絕對路徑則交給 Vite 依 `base` 改寫。
- **iOS 幾乎不看 manifest**。圖示走 `apple-touch-icon` 且**必須是 PNG**（放 SVG
  會被忽略，iOS 改用網頁截圖當圖示）；standalone 走
  `apple-mobile-web-app-capable`。後者有個實際好處：它在**純 http 的區網位址下
  也生效**，所以不必先部署就能測。
- **狀態列用 `black-translucent`**，因為它是唯一會讓 `safe-area-inset-top` 不是
  0 的設定，也是原生 iOS 外殼的預設行為。代價是 iOS 會把時間與電量固定畫成白
  色，淺色主題下要另外看一眼——這是刻意留給實機驗收去回答的問題。
- **沒有 service worker**。離線與通知都不是這一步要解的：iOS 的通知就算有 SW
  也還是要推播伺服器，而開發中的快取只會讓人懷疑改的東西沒有生效。所以現在的
  它是「可安裝」，不是「可離線」。

圖示另外畫了 `icon.svg`，因為 `tomato.svg` 是**平鋪用的浮水印**——兩顆不同大小
的番茄，刻意錯開讓 repeat 不像格子——當圖示並不合適。新的那張是同一套筆法的單
顆置中版本，PNG 由它產出。

順帶記一個踩到的雷：SVG 是當 **XML** 解析的，註解裡出現 `-` `-` 連寫是致命錯
誤而不是警告，整張圖會解不開。第一版的註解裡寫了 CSS 變數名，就是這樣壞的。
